...
use astrix_consensus_core::{api::BlockValidationFutures, block::Block, blockstatus::BlockStatus, errors::block::RuleError};
use astrix_consensusmanager::{BlockProcessingBatch, ConsensusProxy};
use astrix_core::debug;
use astrix_hashes::Hash;
use astrix_p2p_lib::{
    common::ProtocolError,
    dequeue, dequeue_with_timeout, make_message, make_request,
    pb::{astrixd_message::Payload, InvRelayBlockMessage, RequestBlockLocatorMessage, RequestRelayBlocksMessage},
    IncomingRoute, Router, SharedIncomingRoute,
};
use astrix_utils::channel::{JobSender, JobTrySendError as TrySendError};
...
impl HandleRelayInvsFlow {
    ...
    async fn check_orphan_resolution_range(
        ...
        // with current syncer-side implementations (in both go-astrix and this codebase) we could query only the last one,
