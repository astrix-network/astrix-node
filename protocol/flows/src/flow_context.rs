...
use astrix_connectionmanager::ConnectionManager;
use astrix_consensus_core::api::{BlockValidationFuture, BlockValidationFutures};
use astrix_consensus_core::block::Block;
use astrix_consensus_core::config::Config;
use astrix_consensus_core::errors::block::RuleError;
use astrix_consensus_core::tx::{Transaction, TransactionId};
use astrix_consensus_notify::{
    notification::{Notification, PruningPointUtxoSetOverrideNotification},
    root::ConsensusNotificationRoot,
};
use astrix_consensusmanager::{BlockProcessingBatch, ConsensusInstance, ConsensusManager, ConsensusProxy};
use astrix_core::{
    debug, info,
    astrixd_env::{name, version},
    task::tick::TickService,
};
use astrix_core::{time::unix_now, warn};
use astrix_hashes::Hash;
use astrix_mining::manager::MiningManagerProxy;
use astrix_mining::mempool::tx::{Orphan, Priority};
use astrix_notify::notifier::Notify;
use astrix_p2p_lib::{
    common::ProtocolError,
    convert::model::version::Version,
    make_message,
    pb::{astrixd_message::Payload, InvRelayBlockMessage},
    ConnectionInitializer, Hub, AstrixdHandshake, PeerKey, PeerProperties, Router,
};
use astrix_utils::iter::IterExtensions;
...
impl FlowContext {
    ...
    ///
    /// _GO-ASTRIXD: OnNewBlock + broadcastTransactionsAfterBlockAdded_
