...
use crate::pb::{astrixd_message::Payload, ReadyMessage, VerackMessage, VersionMessage};
use crate::{common::ProtocolError, dequeue_with_timeout, make_message};
use crate::{IncomingRoute, AstrixdMessagePayloadType, Router};
use astrix_core::debug;

/// Implements the Astrix peer-to-peer handshake protocol
pub struct AstrixdHandshake<'a> {
    router: &'a Router,
...
impl<'a> AstrixdHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![AstrixdMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![AstrixdMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![AstrixdMessagePayloadType::Ready]),
        }
    }
