use crate::{common::ProtocolError, pb::AstrixdMessage, ConnectionInitializer, Peer, Router};
use astrix_core::{debug, info, warn};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: AstrixdMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: AstrixdMessage) {
    ...
    pub async fn broadcast_to_some_peers(&self, msg: AstrixdMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<AstrixdMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();
