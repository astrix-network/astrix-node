...
`astrix-cli` crate provides cli-driven RPC interface to the node and a
terminal interface to the Astrix Node Wallet runtime. These wallets are
compatible with WASM SDK Wallet API and Astrix NG projects.
...
wRPC
  ...
  wRPC subsystem is disabled by default in `astrixd` and can be enabled via:
  ...
  **Sidenote:**
  ...
  Astrix Node integrates an optional wRPC
  ...
  Astrix data structures and is data-structure-version agnostic. You can connect to the
...
Logging in `astrixd` and `simpa` can be [filtered](https://docs.rs/env_logger/0.10.0/env_logger/#filtering-results) by either:
...
Heap-profiling in `astrixd` and `simpa` can be done by enabling `heap` feature and profile using the `--features` argument
