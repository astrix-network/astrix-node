...
use astrix_consensus_core::BlockHashSet;
use astrix_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use astrix_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use astrix_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use astrix_database::prelude::{DirectWriter, MemoryWriter};
use astrix_database::prelude::{ReadLock, StoreError};
use astrix_database::prelude::{StoreResult, DB};
use astrix_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; astrix_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use astrix_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(astrix_database::prelude::ConnBuilder::default().with_files_limit(10));
