use crate::imports::*;
use astrix_daemon::AstrixdConfig;
...
pub struct Node {
    settings: SettingsStore<AstrixdSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("astrixd").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<AstrixCli>() {
            ctx.daemons().clone().astrixd.as_ref().map(|_| "node")
        } else {
            None
        }
    }
    ...
    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Manage the local Astrix node instance"
    }

    async fn start(self: Arc<Self>, _ctx: &Arc<dyn Context>) -> cli::Result<()> {
        self.settings.try_load().await.ok();
        if let Some(mute) = self.settings.get(AstrixdSettings::Mute) {
    ...
    async fn handle(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, cmd: &str) -> cli::Result<()> {
        let ctx = ctx.clone().downcast_arc::<AstrixCli>()?;
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<AstrixCli>) -> Result<AstrixdConfig> {
        let location: String = self
            .settings
            .get(AstrixdSettings::Location)
        ...
        let mute = false;
        let config = AstrixdConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<AstrixCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let astrixd = ctx.daemons().astrixd();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting astrix node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting astrix node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                astrixd.configure(self.create_config(&ctx).await?).await?;
                astrixd.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // astrixd.mute(mute).await?;
                self.settings.set(AstrixdSettings::Mute, mute).await?;
            }
            "status" => {
                let status = astrixd.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                astrixd.configure(self.create_config(&ctx).await?).await?;
                let version = astrixd.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<AstrixCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Astrixd executable (binary) location"),
                ("version", "Display Astrixd executable version"),
                ("start", "Start the local Astrix node instance"),
                ("stop", "Stop the local Astrix node instance"),
                ("restart", "Restart the local Astrix node instance"),
                ("kill", "Kill the local Astrix node instance"),
                ("status", "Get the status of the local Astrix node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<AstrixCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = astrix_daemon::locate_binaries(root.as_str(), "astrixd").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No astrixd binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a astrixd binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(AstrixdSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(AstrixdSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/astrixd'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<AstrixCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Astrixd has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Astrixd error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
