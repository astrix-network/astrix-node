use crate::imports::*;
use astrix_daemon::{locate_binaries, CpuMinerConfig};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "astrix-cpu-miner").await {
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<AstrixCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<AstrixCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<AstrixCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = astrix_daemon::locate_binaries(root.as_str(), "astrix-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No astrix-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select astrix-cpu-miner binary", &binaries).await? {
