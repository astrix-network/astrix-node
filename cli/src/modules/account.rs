use astrix_wallet_core::account::BIP32_ACCOUNT_KIND;
use astrix_wallet_core::account::LEGACY_ACCOUNT_KIND;
use astrix_wallet_core::account::MULTISIG_ACCOUNT_KIND;
        ...
        match action.as_str() {
            ...
            "import" => {
                if argv.is_empty() {
                    ...
                    ctx.term().help(
                        &[
                            ("account import legacy-data", "Import KDX keydata file or astrixnet web wallet data on the same domain"),
                            (
                                "account import mnemonic bip32",
                                "Import Bip32 (12 or 24 word mnemonics used by astrixwallet)",
                            ),
                            (
                                "account import mnemonic legacy",
                                "Import accounts 12 word mnemonic used by legacy applications (KDX and astrixnet web wallet)",
                            ),
                            (
                ...
                match import_kind.as_ref() {
                    "legacy-data" => {
                        ...
                        if exists_legacy_v0_keydata().await? {
                                ...
                                .import_legacy_keydata(
                                    ...
                                    Some(Arc::new(move |processed: usize, _, balance, txid| {
                                        if let Some(txid) = txid {
                                            tprintln!(
                                                ctx_,
                                                "Scan detected {} AIX at index {}; transfer txid: {}",
                                                sompi_to_astrix_string(balance),
                                                processed,
                                                txid
                                            );
                                        } else if processed > 0 {
                            ...
                            return Err("'astrixnet' web wallet storage not found at this domain name".into());
                        } else {
                    ...
                    "mnemonic" => {
                        if argv.is_empty() {
                            ...
                            tprintln!(ctx, "please use 'legacy' for 12-word KDX and astrixnet web wallet mnemonics\r\n");
                            return Ok(());
                        }
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<AstrixCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (KDX and astrixnet web wallet). Use 'account import' for additional help.",
                ),
            ...
            .derivation_scan(
                ...
                Some(Arc::new(move |processed: usize, _, balance, txid| {
                    if let Some(txid) = txid {
                        ...
                        tprintln!(ctx_, "Scanned {} derivations, found {} AIX", processed, sompi_to_astrix_string(balance));
                    }
                })),
            )
            .await?;
