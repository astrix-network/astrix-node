...
use astrix_wallet_core::storage::{make_filename, Hint};

pub(crate) async fn create(ctx: &Arc<AstrixCli>, name: Option<&str>, import_with_mnemonic: bool) -> Result<()> {
    let term = ctx.term();
    ...
    if let Err(err) = wallet.network_id() {
        tprintln!(ctx);
        tprintln!(ctx, "Before creating a wallet, you need to select a Astrix network.");
    ...
    if !import_with_mnemonic {
        ...
        tpara!(
            ...
            the Astrix stored in it. Keep your mnemonic safe. Write it down and \
