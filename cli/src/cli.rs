...
use astrix_daemon::{DaemonEvent, DaemonKind, Daemons};
use astrix_wallet_core::rpc::DynRpcApi;
use astrix_wallet_core::storage::{IdT, PrvKeyDataInfo};
use astrix_wrpc_client::AstrixRpcClient;
...
impl From<&AstrixCli> for Arc<Terminal> {
    fn from(ctx: &AstrixCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<AstrixCli> for AstrixCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for AstrixCli {
...
impl AstrixCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                astrix_core::log::init_logger(None, "info");
            } else {
                astrix_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let astrix_cli = Arc::new(AstrixCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(astrix_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                astrix_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<AstrixRpcClient>> {
                ...
                select! {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletPing => {
                                    // log_info!("Astrix NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Astrix NG - received metrics event {metrics:?}")
                                }
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: Astrix node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Astrix node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Astrix node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Astrix node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let astrixd = self.daemons().try_astrixd();
            ...
            if let Some(astrixd) = astrixd.as_ref() {
                astrixd.mute(false).await?;
                astrixd.stop().await?;
            }
...
impl Cli for AstrixCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "astrix" {
                prompt.push(title);
            }
...
impl cli::Context for AstrixCli {
...
pub async fn astrix_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    AstrixCli::init();

    let options = Options::new(terminal_options, None);
    let cli = AstrixCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Astrix Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
