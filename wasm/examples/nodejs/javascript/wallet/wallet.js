...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToAstrixString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        list.push({
                            ...
                            Mature: sompiToAstrixString(data.balance.mature),
                            Pending: sompiToAstrixString(data.balance.pending),
                            Outgoing: sompiToAstrixString(data.balance.outgoing),
        ...
        // Send astrix to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            accountId: firstAccount.accountId,
            priorityFeeSompi: astrixToSompi("0.001"),
            destination:[{
                address: firstAccount.changeAddress,
                amount: astrixToSompi("1.5")
            }]
        });
        console.log("sendResult", sendResult);

        // Transfer astrix between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: astrixToSompi("2.4"),
        });
