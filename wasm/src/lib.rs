...
- [**Rustdoc** documentation](https://docs.rs/astrix-wasm/latest/astrix-wasm)
- [**JSDoc** documentation](https://astrix.aspectron.org/jsdoc/)
...
- **RPC API** — [RPC interface bindings](rpc) for the Astrix node using WebSocket (wRPC) connections.
...
- <https://www.npmjs.com/package/astrix>
- <https://www.npmjs.com/package/astrix-wasm>

The `astrix-wasm` module is a pure WASM32 module that includes
the entire wallet framework, but does not support RPC due to an absence
of a native WebSocket in NodeJs environment, while
the `astrix` module includes `websocket` package dependency simulating
the W3C WebSocket and due to this supports RPC.

## Examples

JavaScript examples for using this framework can be found at:
<https://github.com/astrix-network/astrix-node/tree/master/wasm/nodejs>

## WASM32 Binaries

For pre-built browser-compatible WASM32 redistributables of this
framework please see the releases section of the Astrix Node
repository at <https://github.com/astrix-network/astrix-node/releases>.
...
with Astrix RPC implementation.
...
<html>
    <head>
        <script type="module">
            import * as astrix_wasm from './astrix/astrix-wasm.js';
            (async () => {
                const astrix = await astrix_wasm.default('./astrix/astrix-wasm_bg.wasm');
                // ...
            })();
        </script>
    </head>
...
let {RpcClient,Encoding,initConsolePanicHook} = require('./astrix-rpc');
...
compile_error!("`astrix-wasm` crate for WASM32 target must be built with `--features wasm32-sdk|wasm32-rpc|wasm32-core|wasm32-keygen`");
...
cfg_if::cfg_if! {
    ...
    if #[cfg(feature = "wasm32-sdk")] {
        ...
        pub use astrix_addresses::{Address, Version as AddressVersion};
        pub use astrix_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput};
        pub use astrix_pow::wasm::*;

        pub mod rpc {
            //! Astrix RPC interface
            //!

            pub mod messages {
                //! Astrix RPC messages
                pub use astrix_rpc_core::model::message::*;
            }
            pub use astrix_rpc_core::api::rpc::RpcApi;
            pub use astrix_rpc_core::wasm::message::*;

            pub use astrix_wrpc_wasm::client::*;
            pub use astrix_wrpc_wasm::resolver::*;
            pub use astrix_wrpc_wasm::notify::*;
        }

        pub use astrix_consensus_wasm::*;
        pub use astrix_wallet_keys::prelude::*;
        pub use astrix_wallet_core::wasm::*;
        ...
        pub use astrix_rpc_core::api::rpc::RpcApi;
        pub use astrix_rpc_core::wasm::message::*;
        pub use astrix_rpc_core::wasm::message::IPingRequest;
        pub use astrix_wrpc_wasm::client::*;
        pub use astrix_wrpc_wasm::resolver::*;
        pub use astrix_wrpc_wasm::notify::*;
        pub use astrix_wasm_core::types::*;
