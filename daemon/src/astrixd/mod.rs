...
impl Default for AstrixdConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<AstrixdConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: AstrixdConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no astrixd path is specified".to_string()));
        }
...
impl Astrixd {
    pub fn new(args: AstrixdConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: AstrixdConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Astrix node is already running.".to_string()));
            }
        }
        ...
        // let options = AstrixdOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Astrixd binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait AstrixdCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: AstrixdConfig) -> Result<()>;
