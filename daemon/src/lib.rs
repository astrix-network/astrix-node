...
pub use astrixd::{Astrixd, AstrixdConfig, AstrixdCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../astrix-cpu-miner/target/debug",
    "../../astrix-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub astrixd: Option<Arc<dyn AstrixdCtl + Send + Sync + 'static>>,
    // pub astrixd_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { astrixd: None, cpu_miner: None }
    }

    pub fn with_astrixd(mut self, astrixd: Arc<dyn AstrixdCtl + Send + Sync + 'static>) -> Self {
        self.astrixd = Some(astrixd);
        self
    }
    ...
    pub fn astrixd(&self) -> Arc<dyn AstrixdCtl + Send + Sync + 'static> {
        self.astrixd.as_ref().expect("accessing Daemons::astrixd while astrixd option is None").clone()
    }

    pub fn try_astrixd(&self) -> Option<Arc<dyn AstrixdCtl + Send + Sync + 'static>> {
        self.astrixd.clone()
    }
