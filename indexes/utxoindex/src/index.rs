...
use astrix_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use astrix_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use astrix_core::{info, trace};
use astrix_database::prelude::{StoreError, StoreResult, DB};
use astrix_hashes::Hash;
use astrix_index_core::indexed_utxos::BalanceByScriptPublicKey;
use astrix_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-astrixd), for quicker resets, while still having a low memory footprint.
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<astrix_consensus_core::tx::TransactionOutpoint>> {
