...
use astrix_wrpc_server::{
    connection::Connection,
    router::Router,
    server::Server,
    service::{AstrixRpcHandler, Options},
};
use result::Result;
use std::sync::Arc;
use workflow_log::*;
...
struct Args {
    ...
    /// proxy:port for gRPC server (grpc://127.0.0.1:34150)
    #[clap(name = "grpc")]
...
async fn main() -> Result<()> {
    ...
    let astrixd_port = network_type.default_rpc_port();
    ...
    let options = Arc::new(Options {
        ...
        grpc_proxy_address: Some(grpc_proxy_address.unwrap_or_else(|| format!("grpc://127.0.0.1:{astrixd_port}"))),
        verbose,
        // ..Options::default()
    });
    log_info!("");
    ...
    let rpc_handler = Arc::new(AstrixRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("Astrix wRPC server is listening on {}", options.listen_address);
