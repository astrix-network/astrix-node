...
use astrix_addresses::{Address, AddressOrStringArrayT};
use astrix_consensus_client::UtxoEntryReference;
use astrix_consensus_core::network::{NetworkType, NetworkTypeT};
use astrix_notify::connection::ChannelType;
use astrix_notify::events::EventType;
use astrix_notify::listener;
use astrix_notify::notification::Notification as NotificationT;
use astrix_rpc_core::api::ctl;
pub use astrix_rpc_core::wasm::message::*;
pub use astrix_rpc_macros::{
    build_wrpc_wasm_bindgen_interface, build_wrpc_wasm_bindgen_subscriptions, declare_typescript_wasm_interface as declare,
};
use astrix_wasm_core::events::{get_event_targets, Sink};
...
pub struct Inner {
    ...
    notification_channel: Channel<astrix_rpc_core::Notification>,
}
...
///
///
/// Astrix RPC client uses ([wRPC](https://github.com/workflow-rs/workflow-rs/tree/master/rpc))
/// interface to connect directly with Astrix Node. wRPC supports
/// two types of encodings: `borsh` (binary, default) and `json`.
///
/// There are two ways to connect: Directly to any Astrix Node or to a
...
/// ```
///
/// **Connecting to a Astrix Node directly**
///
/// ```javascript
/// let rpc = new RpcClient({
...
/// to re-register event listeners, but your have to re-subscribe for Astrix node
/// notifications:
///
/// ```typescript
/// rpc.addEventListener("connect", async (event) => {
...
impl RpcClient {
    pub fn new(config: Option<RpcConfig>) -> Result<RpcClient> {
        ...
        let client = Arc::new(
            AstrixRpcClient::new(encoding, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
...
impl RpcClient {
    ...
    /// Connect to the Astrix RPC server. This function starts a background
    ...
    /// Disconnect from the Astrix RPC server.
    ...
    /// however, you have to re-subscribe for Astrix node notifications. As such, it is recommended
...
impl RpcClient {
    pub fn new_with_rpc_client(client: Arc<AstrixRpcClient>) -> RpcClient {
    ...
    pub fn client(&self) -> &Arc<AstrixRpcClient> {
        &self.inner.client
    }
                ...
                select_biased! {
                    ...
                    msg = notification_receiver.recv().fuse() => {
                        if let Ok(notification) = &msg {
                            match &notification {
                                astrix_rpc_core::Notification::UtxosChanged(utxos_changed_notification) => {
...
impl RpcClient {
    ...
    pub fn parse_url(url: &str, encoding: Encoding, network: NetworkId) -> Result<String> {
        let url_ = AstrixRpcClient::parse_url(url.to_string(), encoding, network.into())?;
        Ok(url_)
    }
}
...
impl RpcClient {
    ...
    /// Difficulty Adjustment Algorithm (DAA) score changes in the Astrix BlockDAG.
    ...
    /// Astrix BlockDAG. The event notification will be scoped to the
    ...
    /// chain changes in the Astrix BlockDAG.
...
build_wrpc_wasm_bindgen_subscriptions!([
    ...
    /// block is added to the Astrix BlockDAG.
    BlockAdded,
    ...
    /// conflict occurs in the Astrix BlockDAG.
    FinalityConflict,
    ...
    /// conflict in the Astrix BlockDAG is resolved.
    ...
    /// score of the sink block changes in the Astrix BlockDAG.
    ...
    /// UTXO set override for the pruning point changes in the Astrix BlockDAG.
    ...
    /// template is generated for mining in the Astrix BlockDAG.
    NewBlockTemplate,
]);
...
build_wrpc_wasm_bindgen_interface!(
    [
        ...
        /// Retrieves the current number of blocks in the Astrix BlockDAG.
        ...
        /// structure of the Astrix BlockDAG.
        ...
        GetBlockDagInfo,
        /// Returns the total current coin supply of Astrix network.
        /// Returned information: Total coin supply.
        GetCoinSupply,
        /// Retrieves information about the peers connected to the Astrix node.
        /// Returned information: Peer ID, IP address and port, connection
        /// status, protocol version.
        GetConnectedPeerInfo,
        /// Retrieves general information about the Astrix node.
        /// Returned information: Version of the Astrix node, protocol
        /// version, network identifier.
        /// This call is primarily used by gRPC clients.
        /// For wRPC clients, use {@link RpcClient.getServerInfo}.
        GetInfo,
        /// Provides a list of addresses of known peers in the Astrix
        ...
        GetSinkBlueScore,
        /// Tests the connection and responsiveness of a Astrix node.
        /// Returned information: None.
        Ping,
        /// Gracefully shuts down the Astrix node.
        /// Returned information: None.
        Shutdown,
        /// Retrieves information about the Astrix server.
        /// Returned information: Version of the Astrix server, protocol
        /// version, network identifier.
        GetServerInfo,
        /// Obtains basic information about the synchronization status of the Astrix node.
        /// Returned information: Syncing status.
        GetSyncStatus,
    ],
    [
        // functions with `request` argument
        /// Adds a peer to the Astrix node's list of known peers.
        /// Returned information: None.
        AddPeer,
        /// Bans a peer from connecting to the Astrix node for a specified duration.
        /// Returned information: None.
        Ban,
        ...
        /// Retrieves the balance of a specific address in the Astrix BlockDAG.
        /// Returned information: Balance of the address.
        GetBalanceByAddress,
        /// Retrieves balances for multiple addresses in the Astrix BlockDAG.
        /// Returned information: Balances of the addresses.
        GetBalancesByAddresses,
        /// Retrieves a specific block from the Astrix BlockDAG.
        /// Returned information: Block information.
        GetBlock,
        /// Retrieves multiple blocks from the Astrix BlockDAG.
        ...
        GetCurrentNetwork,
        /// Retrieves block headers from the Astrix BlockDAG.
        /// Returned information: List of block headers.
        GetHeaders,
        /// Retrieves mempool entries from the Astrix node's mempool.
        ...
        GetMempoolEntry,
        /// Retrieves information about a subnetwork in the Astrix BlockDAG.
        ...
        /// Resolves a finality conflict in the Astrix BlockDAG.
        /// Returned information: None.
        ResolveFinalityConflict,
        /// Submits a block to the Astrix network.
        /// Returned information: None.
        SubmitBlock,
        /// Submits a transaction to the Astrix network.
        /// Returned information: None.
        SubmitTransaction,
        /// Unbans a previously banned peer, allowing it to connect
        /// to the Astrix node again.
        /// Returned information: None.
        Unban,
    ]
);
