...
use crate::imports::*;
use js_sys::Array;
pub use astrix_rpc_macros::declare_typescript_wasm_interface as declare;
use astrix_wrpc_client::node::NodeDescriptor;
use astrix_wrpc_client::Resolver as NativeResolver;
use serde::ser;
...
///
/// Resolver is a client for obtaining public Astrix wRPC URL.
///
/// Resolver queries a list of public Astrix Resolver URLs using HTTP to fetch
...
#[wasm_bindgen]
impl Resolver {
    /// List of public Astrix Resolver URLs.
    #[wasm_bindgen(getter)]
    pub fn urls(&self) -> ResolverArrayT {
        Array::from_iter(self.resolver.urls().iter().map(|v| JsValue::from(v.as_str()))).unchecked_into()
    }

    /// Fetches a public Astrix wRPC endpoint for the given encoding and network identifier.
    ...
    /// Fetches a public Astrix wRPC endpoint URL for the given encoding and network identifier.
    ...
    /// Connect to a public Astrix wRPC endpoint for the given encoding and network identifier
