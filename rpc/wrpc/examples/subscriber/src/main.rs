// Basic example of a Astrix wRPC client that connects to a node
...
// Astrix RPC primitives
use astrix_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use astrix_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Astrix RPC client instance using Borsh encoding.
        let client = Arc::new(AstrixRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
