...
use astrix_rpc_core::api::ops::RpcApiOps;
...
/// ### AstrixRpcHandler
///
/// [`AstrixRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct AstrixRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl AstrixRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        AstrixRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for AstrixRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "astrix" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<AstrixRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(AstrixRpcHandler::new(tasks, *encoding, core_service, options.clone()));
