use astrix_consensus_core::{subnets::SubnetworkConversionError, tx::TransactionId};
...
pub enum RpcError {
    ...
    #[error(transparent)]
    AddressError(#[from] astrix_addresses::AddressError),

    #[error(transparent)]
    NetworkTypeError(#[from] astrix_consensus_core::network::NetworkTypeError),

    #[error(transparent)]
    NetworkIdError(#[from] astrix_consensus_core::network::NetworkIdError),

    #[error(transparent)]
    NotificationError(#[from] astrix_notify::error::Error),

    #[error(transparent)]
    MiningManagerError(#[from] astrix_mining_errors::manager::MiningManagerError),

    #[error(transparent)]
    ConsensusError(#[from] astrix_consensus_core::errors::consensus::ConsensusError),

    #[error(transparent)]
    ScriptClassError(#[from] astrix_txscript::script_class::Error),
