...
use astrix_core::{debug, error, trace};
use astrix_grpc_core::{
    channel::NotificationChannel,
    ops::AstrixdPayloadOps,
    protowire::{astrixd_request, rpc_client::RpcClient, GetInfoRequestMessage, AstrixdRequest, AstrixdResponse},
...
use astrix_utils::{channel::Channel, triggers::DuplexTrigger};
use astrix_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(AstrixdPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type AstrixdRequestSender = async_channel::Sender<AstrixdRequest>;
type AstrixdRequestReceiver = async_channel::Receiver<AstrixdRequest>;
...
/// Data flow:
/// ```
/// //   AstrixdRequest -> request_send -> stream -> AstrixdResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: AstrixdRequestSender,
    request_receiver: AstrixdRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<AstrixdResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual AstrixdRequest to AstrixdResponse stream
        let mut stream: Streaming<AstrixdResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: AstrixdPayloadOps, request: impl Into<AstrixdRequest>) -> Result<AstrixdResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: AstrixdRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<AstrixdResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = astrixd_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = astrixd_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
