...
use super::{
    ...
    interface::{Interface, AstrixdMethod, AstrixdRoutingPolicy},
    method::Method,
};
use crate::{
...
use astrix_grpc_core::protowire::{astrixd_request::Payload, *};
use astrix_grpc_core::{ops::AstrixdPayloadOps, protowire::NotifyFinalityConflictResponseMessage};
use astrix_notify::{scope::FinalityConflictResolvedScope, subscriber::SubscriptionManager};
use astrix_rpc_core::{SubmitBlockRejectReason, SubmitBlockReport, SubmitBlockResponse};
use astrix_rpc_macros::build_grpc_server_interface;

pub struct Factory {}

impl Factory {
    pub fn new_handler(
        rpc_op: AstrixdPayloadOps,
    ...
    pub fn new_interface(server_ctx: ServerContext, network_bps: u64) -> Interface {
        ...
        let method: AstrixdMethod = Method::new(|server_ctx: ServerContext, connection: Connection, request: AstrixdRequest| {
            Box::pin(async move {
                let mut response: AstrixdResponse = match request.payload {
                    Some(Payload::NotifyFinalityConflictRequest(ref request)) => {
                        match astrix_rpc_core::NotifyFinalityConflictRequest::try_from(request) {
                            Ok(request) => {
        ...
        interface.replace_method(AstrixdPayloadOps::NotifyFinalityConflict, method);
        ...
        interface.set_method_properties(
            ...
            AstrixdRoutingPolicy::DropIfFull(Arc::new(Box::new(|_: &AstrixdRequest| {
