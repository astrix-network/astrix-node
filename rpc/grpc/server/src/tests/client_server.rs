...
use astrix_notify::scope::{NewBlockTemplateScope, Scope};
use astrix_rpc_core::api::rpc::RpcApi;
use astrix_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    astrix_core::log::try_init_logger("info, astrix_grpc_core=trace, astrix_grpc_server=trace, astrix_grpc_client=trace");
