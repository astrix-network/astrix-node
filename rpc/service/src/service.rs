...
use astrix_consensus_core::api::counters::ProcessingCounters;
use astrix_consensus_core::errors::block::RuleError;
...
use astrix_index_core::indexed_utxos::BalanceByScriptPublicKey;
use astrix_index_core::{
...
use astrix_mining::model::tx_query::TransactionQuery;
use astrix_mining::{manager::MiningManagerProxy, mempool::tx::Orphan};
use astrix_notify::listener::ListenerLifespan;
use astrix_notify::subscription::context::SubscriptionContext;
use astrix_notify::subscription::{MutationPolicies, UtxosChangedMutationPolicy};
use astrix_notify::{
...
use astrix_p2p_flows::flow_context::FlowContext;
use astrix_p2p_lib::common::ProtocolError;
use astrix_perf_monitor::{counters::CountersSnapshot, Monitor as PerfMonitor};
use astrix_rpc_core::{
    api::{
        ops::RPC_API_VERSION,
...
use astrix_txscript::{extract_script_pub_key_address, pay_to_address_script};
use astrix_utils::{channel::Channel, triggers::SingleTrigger};
use astrix_utils_tower::counters::TowerConnectionCounters;
use astrix_utxoindex::api::UtxoIndexProxy;
use std::{
    collections::HashMap,
    iter::once,
...
/// A service implementing the Rpc API at astrix_rpc_core level.
///
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. If you are on TN11 this is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(&self, request: GetBlockTemplateRequest) -> RpcResult<GetBlockTemplateResponse> {
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(astrix_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }

        // Build block template
        let script_public_key = astrix_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: astrix_hashes::Hash = astrix_hashes::Hash::from_bytes([0xff; astrix_hashes::HASH_SIZE]);
