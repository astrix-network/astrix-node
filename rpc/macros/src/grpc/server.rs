...
struct RpcTable {
    ...
    astrixd_request_type: Expr,
    astrixd_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, AstrixdRequestType, AstrixdResponseType, AstrixdPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let astrixd_request_type = iter.next().unwrap().clone();
        let astrixd_response_type = iter.next().unwrap().clone();
        ...
        Ok(RpcTable {
            ...
            server_ctx_type,
            connection_ctx_type,
            astrixd_request_type,
            astrixd_response_type,
            payload_ops,
            handlers,
        })
    }
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let astrixd_request_type = &self.astrixd_request_type;
        let astrixd_response_type = &self.astrixd_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #astrixd_request_type, #astrixd_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #astrixd_request_type| {
                                Box::pin(async move {
                                    let mut response: #astrixd_response_type = match request.payload {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #astrixd_request_type, #astrixd_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #astrixd_request_type| {
                                Box::pin(async move {
                                    let mut response: #astrixd_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match astrix_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {
