...
use astrix_bip32::{ExtendedPrivateKey, Language, Mnemonic, Prefix, SecretKey};
use md5::Md5;
...
// ---
// {"type":"astrix-wallet","encryption":"default","version":1,"generator":"pwa","wallet":{"mnemonic":"hex"}}
...
fn legacy_v0_keydata_location() -> Result<(PathBuf, Options)> {
    let filename = if runtime::is_windows() {
        ...
        fs::resolve_path(&format!("{appdata}/Astrix/astrix.kpk"))?
    } else if runtime::is_macos() {
        fs::resolve_path("~/Library/Application Support/Astrix/astrix.kpk")?
    } else {
        fs::resolve_path("~/.astrix/astrix.kpk")?
    };

    let options = workflow_store::fs::Options::with_local_storage_key("astrix-wallet");
