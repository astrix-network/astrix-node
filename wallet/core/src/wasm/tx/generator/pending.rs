...
use astrix_consensus_client::{numeric, string};
use astrix_consensus_client::{ITransaction, Transaction};
use astrix_wallet_keys::privatekey::PrivateKey;
    ...
    pub fn sign(&self, js_value: PrivateKeyArrayT) -> Result<()> {
        if let Ok(keys) = js_value.dyn_into::<Array>() {
            let keys = keys
                ...
                .collect::<std::result::Result<Vec<_>, astrix_wallet_keys::error::Error>>()?;
