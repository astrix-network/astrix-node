//!
//! API trait for interfacing with the Astrix wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `astrix`.
    ///
    ...
    /// applications such as KDX and astrixnet.io web wallet using a deprecated derivation path
