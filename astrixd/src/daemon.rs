...
use astrix_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use astrix_core::{core::Core, info, trace};
use astrix_core::{astrixd_env::version, task::tick::TickService};
use astrix_database::prelude::CachePolicy;
use astrix_grpc_server::service::GrpcService;
use astrix_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use astrix_rpc_service::service::RpcCoreService;
use astrix_txscript::caches::TxScriptCacheCounters;
use astrix_utils::networking::ContextualNetAddress;
use astrix_utils_tower::counters::TowerConnectionCounters;

use astrix_addressmanager::AddressManager;
use astrix_consensus::{consensus::factory::Factory as ConsensusFactory, pipeline::ProcessingCounters};
use astrix_consensus::{
...
use astrix_p2p_flows::{flow_context::FlowContext, service::P2pService};

use astrix_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use astrix_utxoindex::{api::UtxoIndexProxy, UtxoIndex};
use astrix_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the astrixd process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the astrixd
/// process. (Astrix Node will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("astrix-node");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".astrix-node");
}
...
/// the application folder that contains astrixd logs and the database.
...
impl Runtime {
    pub fn from_args(args: &Args) -> Self {
        ...
        astrix_core::log::init_logger(log_dir.as_deref(), &args.log_level);
...
do you confirm? (answer y/n or pass --yes to the Astrixd command line to confirm all interactive questions)";
    ...
    let mut meta_db = astrix_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = astrix_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from astrixd DB version
    ...
    {
        let msg =
            "Node database is from a different Astrixd *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
        get_user_approval_or_exit(msg, args.yes);

        info!("Deleting databases from previous Astrixd version");
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = astrix_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            trace!("[{}] {}", astrix_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            trace!("[{}] {}", astrix_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            trace!("[{}] heap stats: {:?}", astrix_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = astrix_database::prelude::ConnBuilder::default()
