extern crate astrix_consensus;
extern crate astrix_core;
extern crate astrix_hashes;

use std::sync::Arc;

use astrix_alloc::init_allocator_with_default_settings;
use astrix_core::{info, signals::Signals};
...
pub fn main() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("astrixd-heap.json").build();
    ...
    match fd_budget::try_set_fd_limit(DESIRED_DAEMON_SOFT_FD_LIMIT) {
        Ok(limit) => {
            if limit < MINIMUM_DAEMON_SOFT_FD_LIMIT {
                ...
                println!("The astrixd node requires a setting of at least {DESIRED_DAEMON_SOFT_FD_LIMIT} to operate properly.");
