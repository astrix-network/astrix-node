...
use astrix_notify::{events::EventType, scope::Scope, subscription::Command};
use astrix_rpc_core::RpcResult;
use std::{
...
impl ListeningClient {
    pub async fn connect(astrixd: &Daemon) -> Self {
        let client = astrixd.new_multi_listener_client().await;
