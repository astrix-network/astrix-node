...
use astrix_consensus::params::SIMNET_PARAMS;
use astrix_consensusmanager::ConsensusManager;
use astrix_core::{task::runtime::AsyncRuntime, trace};
use astrix_grpc_client::GrpcClient;
use astrix_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use astrix_rpc_core::{api::rpc::RpcApi, Notification, RpcTransactionId};
use astrix_txscript::pay_to_address_script;
use astrixd_lib::args::Args;
use rand::thread_rng;
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  astrix_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut astrixd1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = astrixd1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut astrixd2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = astrixd2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut astrixd1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut astrixd2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = astrixd1.start().await;
    let rpc_client2 = astrixd2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", astrixd1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(astrixd1.network.into(), astrix_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2.get_virtual_chain_from_block(astrix_consensus::params::SIMNET_GENESIS.hash, true).await.unwrap();
...
/// `cargo test --release --package astrix-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("astrix-testing-integration-heap.json").build();

    astrix_core::log::try_init_logger(
        "INFO,astrix_testing_integration=trace,astrix_notify=debug,astrix_rpc_core=debug,astrix_grpc_client=debug",
    );
    ...
    let miner_address =
        Address::new(astrixd1.network.into(), astrix_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(astrixd1.network.into(), astrix_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(astrixd1.network.into(), astrix_addresses::Version::PubKey, &[0; 32]);
    ...
    let mut clients = vec![ListeningClient::connect(&astrixd2).await, ListeningClient::connect(&astrixd1).await];
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    astrix_core::log::try_init_logger("info,astrix_grpc_core=trace,astrix_grpc_server=trace,astrix_grpc_client=trace,astrix_core=trace");
    ...
    {
        let total_fd_limit = 10;
        let mut astrixd1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = astrixd1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = astrixd1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&astrixd1.core);
