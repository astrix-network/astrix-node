...
use astrix_alloc::init_allocator_with_default_settings;
use astrix_consensus::config::genesis::GENESIS;
use astrix_consensus::config::{Config, ConfigBuilder};
use astrix_consensus::consensus::factory::Factory as ConsensusFactory;
use astrix_consensus::consensus::test_consensus::{TestConsensus, TestConsensusFactory};
use astrix_consensus::model::stores::block_transactions::{
    BlockTransactionsStore, BlockTransactionsStoreReader, DbBlockTransactionsStore,
};
use astrix_consensus::model::stores::ghostdag::{GhostdagStoreReader, KType as GhostdagKType};
use astrix_consensus::model::stores::headers::HeaderStoreReader;
use astrix_consensus::model::stores::reachability::DbReachabilityStore;
use astrix_consensus::model::stores::relations::DbRelationsStore;
use astrix_consensus::model::stores::selected_chain::SelectedChainStoreReader;
use astrix_consensus::params::{Params, DEVNET_PARAMS, MAINNET_PARAMS, MAX_DIFFICULTY_TARGET, MAX_DIFFICULTY_TARGET_AS_F64};
use astrix_consensus::pipeline::monitor::ConsensusMonitor;
use astrix_consensus::pipeline::ProcessingCounters;
use astrix_consensus::processes::reachability::tests::{DagBlock, DagBuilder, StoreValidationExtensions};
use astrix_consensus::processes::window::{WindowManager, WindowType};
use astrix_consensus_core::api::{BlockValidationFutures, ConsensusApi};
use astrix_consensus_core::block::Block;
use astrix_consensus_core::blockhash::new_unique;
use astrix_consensus_core::blockstatus::BlockStatus;
use astrix_consensus_core::constants::{BLOCK_VERSION, STORAGE_MASS_PARAMETER};
use astrix_consensus_core::errors::block::{BlockProcessResult, RuleError};
use astrix_consensus_core::header::Header;
use astrix_consensus_core::network::{NetworkId, NetworkType::Mainnet};
use astrix_consensus_core::subnets::SubnetworkId;
use astrix_consensus_core::trusted::{ExternalGhostdagData, TrustedBlock};
use astrix_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput, UtxoEntry};
use astrix_consensus_core::{blockhash, hashing, BlockHashMap, BlueWorkType};
use astrix_consensus_notify::root::ConsensusNotificationRoot;
use astrix_consensus_notify::service::NotifyService;
use astrix_consensusmanager::ConsensusManager;
use astrix_core::task::tick::TickService;
use astrix_core::time::unix_now;
use astrix_database::utils::get_astrix_tempdir;
use astrix_hashes::Hash;

use flate2::read::GzDecoder;
use futures_util::future::try_join_all;
use itertools::Itertools;
use astrix_core::core::Core;
use astrix_core::signals::Shutdown;
use astrix_core::task::runtime::AsyncRuntime;
use astrix_core::{assert_match, info};
use astrix_database::create_temp_db;
use astrix_database::prelude::{CachePolicy, ConnBuilder};
use astrix_index_processor::service::IndexService;
use astrix_math::Uint256;
use astrix_muhash::MuHash;
use astrix_notify::subscription::context::SubscriptionContext;
use astrix_txscript::caches::TxScriptCacheCounters;
use astrix_utxoindex::api::{UtxoIndexApi, UtxoIndexProxy};
use astrix_utxoindex::UtxoIndex;
...
async fn json_test(file_path: &str, concurrency: bool) {
    astrix_core::log::try_init_logger("info");
    ...
    let go_params_res: Result<AstrixdGoParams, _> = serde_json::from_str(&first_line);
...
async fn staging_consensus_test() {
    ...
    let db_tempdir = get_astrix_tempdir();
    let db_path = db_tempdir.path().to_owned();
    let consensus_db_dir = db_path.join("consensus");
    let meta_db_dir = db_path.join("meta");

    let meta_db = astrix_database::prelude::ConnBuilder::default().with_db_path(meta_db_dir).with_files_limit(5).build().unwrap();
